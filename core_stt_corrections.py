...
# faster-whisper base/small에서 짧은 한국어 명령이 자주 흔들리는 케이스.
# 일반 텍스트 입력에는 적용하지 말고, 음성 STT 결과에만 적용하는 용도다.
_CHROME_CONFUSIONS = (
    "그럼", "그롬", "그룸", "그름", "구름", "크럼", "크름", "크론", "크롬이", "크로미",
)

# 앱 이름 바로 뒤 명령 동사가 따라올 때만 치환한다.
# 예: "그럼 띄워봐" -> "크롬 띄워봐"
_CHROME_PATTERNS = [
    re.compile(
...
def correct_stt_text(text: str, *, return_reasons: bool = False):
    ...
    핵심 원칙:
    - 텍스트 채팅에는 쓰지 않는다. 음성 STT 결과에만 사용한다.
    - 앱 실행/포커스/검색처럼 명령 동사가 보일 때만 앱명 보정을 한다.
    - 특히 "그럼"은 실제 한국어 단어라서, 명령 동사가 바로 따라올 때만 "크롬"으로 본다.
    """
...
if __name__ == "__main__":
    ...
    for sample in samples:
        print(sample, "=>", correct_stt_text(sample, return_reasons=True))
